...
interface SignUpFormProps {
  lang: Lang;
  onSubmit: (formData: FormData, markSubmitted: () => void) => void;
  isSubmitting: boolean;
}

export default function SignUpForm({ lang, onSubmit, isSubmitting }: SignUpFormProps) {
  ...
  const tierDisplayPrices: Record<string, string> = {
    bronze: 'HK$1,000',
    silver: 'HK$10,000',
    gold: 'HK$100K',
    platinum: 'HK$1M'
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-xs text-zinc-500 mb-1">{labels.jobTitle}</label>
          <input
            type="text"
