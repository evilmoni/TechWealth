...
import { ShieldCheck, Lock, LogOut, Users, Clock, CheckCircle, XCircle, DollarSign, Eye, Edit3, X } from 'lucide-react';
...
const AdminAccess = ({ onClose }) => {
  ...
  // Simple password (change before production!)
  const ADMIN_PASSWORD = 'techwealth2026';

  useEffect(() => {
    // Check if already logged in
    const isLoggedIn = sessionStorage.getItem('tw_admin_auth');
    if (isLoggedIn === 'true') {
